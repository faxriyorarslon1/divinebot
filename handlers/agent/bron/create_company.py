...
from states.bron import CreateCompanyState, BronState
from utils import get_company


@dp.message_handler(lambda message: not str(message.text).__eq__('/start'), state=CreateCompanyState.inn)
async def create_inn_handler(message: types.Message, state: FSMContext):
