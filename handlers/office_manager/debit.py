...
from excel_utils.product_report import debit_create_excel
from states import BaseState
from states.orders import DebitState


@dp.message_handler(lambda message: (str(message.text).__eq__(latin['QARZDORLIK']) or str(message.text).__eq__(
    russian['QARZDORLIK']) or str(message.text).__eq__(translate_cyrillic_or_latin(latin['QARZDORLIK'], 'cyr'))) and (
...
async def debit_handler(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        text = translate(data['lang'], 'CREATE_DEBIT_TEXT')
...
async def create_excel_file_handler(message: types.Message, state: FSMContext):
    async with state.proxy() as data:
        if message.document.file_name.endswith('xlsx') or message.document.file_name.endswith(
            ...
            Debit(token=data['token'], image=data['excel'], file_name=data['file_name']).create()
            ...
            debit_create_excel(data['file_name'], response.content)
...
#                     state=DebitState.excel)
# async def async_func_any_data_handler(message: types.Message, state: FSMContext):
