...
from states import CreateCompanyBossesState, BaseState, HospitalResidueState, CreateCompanyBossesManagerState, \
    HospitalResidueManagerState


@dp.message_handler(state=CreateCompanyBossesManagerState.director_name)
    ...
    async with state.proxy() as data:
        if not (str(message.text).__eq__(latin['HOME_BACK']) or str(message.text).__eq__(
            ...
            if not (message.text.__eq__(latin['BACK_MENU']) or message.text.__eq__(
                ...
                await CreateCompanyBossesManagerState.director_phone.set()
...
@dp.message_handler(state=CreateCompanyBossesManagerState.director_phone)
    ...
    async with state.proxy() as data:
        if not (str(message.text).__eq__(latin['HOME_BACK']) or str(message.text).__eq__(
            ...
            if not (message.text.__eq__(latin['BACK_MENU']) or message.text.__eq__(
                ...
                await CreateCompanyBossesManagerState.provider_name.set()
                await message.bot.send_message(text=text, chat_id=message.chat.id)
                return
...
@dp.message_handler(state=CreateCompanyBossesManagerState.provider_name)
    ...
    async with state.proxy() as data:
        if not (str(message.text).__eq__(latin['HOME_BACK']) or str(message.text).__eq__(
            ...
            if not (message.text.__eq__(latin['BACK_MENU']) or message.text.__eq__(
                ...
                await CreateCompanyBossesManagerState.provider_phone.set()
                await message.bot.send_message(text=text, chat_id=message.chat.id)
                return
            text = translate(data['lang'], 'CREATE_DIRECTOR_PHONE_NUMBER')
            await CreateCompanyBossesManagerState.director_phone.set()
...
@dp.message_handler(state=CreateCompanyBossesManagerState.provider_phone)
    ...
    async with state.proxy() as data:
        if not (str(message.text).__eq__(latin['HOME_BACK']) or str(message.text).__eq__(
            ...
            if not (message.text.__eq__(latin['BACK_MENU']) or message.text.__eq__(
                ...
                await CreateCompanyBossesManagerState.check.set()
                await message.bot.send_message(text=text, chat_id=message.chat.id,
...
@dp.callback_query_handler(state=CreateCompanyBossesManagerState.check)
